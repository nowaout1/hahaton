...
const INITIAL_USERS: AdminUser[] = [
  { id: 44921, name: "Иван Иванов", alliance: "Альянс Центр", role: "operator", verified: true },
              ...
              >
                ...
                <td className="px-4 py-4">
                  <select
                    value={user.role}
                    onChange={(event) =>
                      changeRole(user.id, event.target.value as AdminUser["role"])
                    }
                ...
                <td className="px-4 py-4">
                  ...
                  >
                    ...
                    <span className={user.verified ? "text-white" : "text-white/38"}>
                ...
                <td className="px-4 py-4">
                  <div className="flex justify-end gap-2">
                    <IconButton
                      label="Смотреть график"
                      tone="view"
                      onClick={() => setActivity(`Открыт график пользователя ${user.name}.`)}
                    >
                      View
                    </IconButton>
                    <IconButton
...
function IconButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className={`rounded-sm border px-3 py-2 text-[10px] font-extrabold uppercase tracking-[0.16em] transition ${
        tone === "view"
          ? "border-white/12 text-white/60 hover:border-[#CCFF00] hover:text-[#CCFF00]"
          : "border-white/12 text-white/60 hover:border-[#FF0064] hover:text-[#FF0064]"
      }`}
    >
      {children}
    </button>
